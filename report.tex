...
\centering
\includegraphics[scale=0.3]{comp_ll_timetaken.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[scale=0.4]{mortcomp_hsmmhmm.pdf}
...
\[
o_{hl_h rsc} = \sum \mathbf{1} \left[ X(t_{j+1})=s,X(t_j)=r \right]
\]
\[
e_{hl_h rsc} = \sum \mathbf{P} (X(t_{j+1})=s|X(t_j)=r)
\]
...
\[
T=\sum_{h}\sum_{l_h}\sum_{r}\sum_{s}\sum_{c}\frac{(o_{hl_h rsc}-e_{hl_h rsc})^2}{e_{hl_h rsc}}
\]
